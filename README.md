...
The stat per operation is inspired by Mark Callaghan, [Small Datum](http://smalldatum.blogspot.com)
...
wMB:     amount data read written to the device(megabytes) per second
...
random(insert:point-query=128:8):
...
sequential(insert:point-query=128:8):
...
query-range(insert:range-query=128:8):
